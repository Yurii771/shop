...
class GoodsController extends AbstractActionController
{
    ...
    public function __construct($em) {
        ...
        $query = $this->_objectManager->createQuery('SELECT u FROM Admin\Entity\Goods u');
        //$this->lastId = count($query->getResult());
      ...
      $categories = $this->sortCategories($rows);
        ...
        if ($this->request->isPost()) {
            ...
            if(!empty($_FILES['photo'])){
                ...
                $id=$this->getLastId()+1;
                $newName =$id.".".$ext;
                $upload=move_uploaded_file($file_info['tmp_name'], 'public/img/goods/'.$newName);
